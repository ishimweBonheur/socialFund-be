CREATE TABLE fund_transactions (
    ...
    type VARCHAR(20) NOT NULL CHECK (type IN ('CONTRIBUTION','ASSISTANCE','ADJUSTMENT','REFUND')),
    ...
    CHECK ((type = 'CONTRIBUTION' AND direction = 'IN' AND contribution_id IS NOT NULL AND assistance_request_id IS NULL)
        OR (type = 'ASSISTANCE' AND direction = 'OUT' AND assistance_request_id IS NOT NULL AND contribution_id IS NULL)
...
CREATE UNIQUE INDEX fund_transactions_one_assistance_idx ON fund_transactions (assistance_request_id) WHERE type = 'ASSISTANCE';
...
CREATE INDEX fund_transactions_assistance_id_idx ON fund_transactions (assistance_request_id) WHERE assistance_request_id IS NOT NULL;
