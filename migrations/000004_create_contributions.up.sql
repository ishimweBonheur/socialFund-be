CREATE TABLE contributions (
    ...
    late_fee_amount NUMERIC(15,2) NOT NULL DEFAULT 0 CHECK (late_fee_amount >= 0), overdue_at TIMESTAMPTZ,
    status VARCHAR(20) NOT NULL DEFAULT 'UPCOMING' CHECK (status IN ('UPCOMING','DUE','OVERDUE','PENDING','APPROVED','REJECTED','FROZEN')),
    frozen_status VARCHAR(20) CHECK (frozen_status IN ('UPCOMING','DUE','OVERDUE','PENDING','REJECTED')),
    ...
    approval_token_action VARCHAR(10) CHECK (approval_token_action IS NULL OR approval_token_action IN ('APPROVE','REJECT')),
...
CREATE UNIQUE INDEX contributions_transaction_reference_unique ON contributions (upper(btrim(transaction_reference))) WHERE transaction_reference IS NOT NULL AND btrim(transaction_reference) <> '';
