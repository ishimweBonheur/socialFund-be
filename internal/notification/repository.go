...
func (r *PostgresRepository) List(ctx context.Context, f Filter) ([]Notification, error) {
	...
	for rows.Next() {
		var n Notification
		if err = rows.Scan(&n.ID, &n.UserID, &n.ContributionID, &n.AssistanceRequestID, &n.Type, &n.Channel, &n.Recipient, &n.Subject, &n.Message, &n.Status, &n.Attempts, &n.LastError, &n.NextRetryAt, &n.SentAt, &n.ReadAt, &n.AttachmentKey, &n.ProofURL, &n.ApproveURL, &n.RejectURL, &n.CreatedAt); err != nil {
			return nil, err
		}
...
func (r *PostgresRepository) ListReady(ctx context.Context, limit int) ([]Notification, error) {
	rows, err := r.db.Query(ctx, `UPDATE notifications SET status='PROCESSING',attempts=attempts+1 WHERE id IN (SELECT id FROM notifications WHERE channel='EMAIL' AND (status='PENDING' OR (status='FAILED' AND next_retry_at<=NOW())) ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT $1) RETURNING id,user_id,contribution_id,assistance_request_id,type,channel,recipient,subject,message,status,attempts,last_error,next_retry_at,sent_at,read_at,attachment_key,proof_url,approve_url,reject_url,created_at`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
